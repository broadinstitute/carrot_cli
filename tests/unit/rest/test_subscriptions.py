...
def find_by_id_data(request):
    ...
    mockito.when(request_handler).find_by_id('subscriptions', request.param["id"]).thenReturn(request.param['return'])
...
def find_data(request):
    ...
    # Mock up request response
    mockito.when(request_handler).find("subscriptions", request.param["params"]).thenReturn(request.param['return'])
    return request.param
